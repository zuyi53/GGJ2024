...
{
    return characterOwner && characterOwner->GetActionsComponent() && characterOwner->GetActionsComponent()->CanExecuteAction(action.ActionTag) && FMath::RandRange(0.f, 100.f) < action.ChancePercentage && ((uint8)characterOwner->GetRelativeTargetDirection(characterOwner->GetTarget()) == (uint8)action.AllowedFromDirection || action.AllowedFromDirection == EActionDirection::EveryDirection);
}
