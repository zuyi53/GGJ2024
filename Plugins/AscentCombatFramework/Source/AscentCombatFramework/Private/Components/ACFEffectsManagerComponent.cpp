...
void UACFEffectsManagerComponent::PlayHitReactionEffect(const FACFDamageEvent& damageEvent)
{
    ...
    if (TryGetDamageFX(damageEvent.HitResponseAction, damageEvent.DamageClass, outFX)) {
      //  outFX.SpawnLocation = damageEvent.hitResult.ImpactPoint;
     //   UACMCollisionsFunctionLibrary::PlayReplicatedEffect(outFX, this);
        ...
        if (effDisp) {
            FActionEffect effect(outFX, ESpawnFXLocation::ESpawnAttachedToSocketOrBone,  damageEvent.hitResult.BoneName);
